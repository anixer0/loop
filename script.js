      ...
      results.forEach((result) => {
        if (result.i && (result.qid === "movie" || result.qid === "tvSeries")) {
          ...
          if (result.qid === "movie" && result.i) {
            imageAndInfo = `<a onClick="setUrl(this); return setVideo(this);" url="imdb=${result.id}&type=movie&title=${result.l.replace(/ /g, "_")}" isWebSeries="false" title="${result.l}"  class="links" IMDB="${result.id}" href="https://player.autoembed.cc/embed/movie/${result.id}?server=3" target="_blank">
            ...
            imageAndInfo = `<a onClick="setUrl(this); return setVideo(this);" url="imdb=${result.id}&season=1&episode=1&title=${result.l.replace(/ /g, "_")}" IMDB="${result.id}" title="${result.l}" isWebSeries="true" class="links" href="https://player.autoembed.cc/embed/tv/${result.id}?server=3/1/1" target="_blank">
                      <img alt="${result.l}" src="${optimisedImageUrl(result.i.imageUrl)}">
                        <div class="info">
                          <h3>${result.l}</h3>
                          <p>${result.s}</p>
...
function setAll(imdb, title, season, episode, type) {
  if (imdb && title && !season && !episode && type) {
    ...
    a.setAttribute("href", " https://embed.smashystream.com/status.php?imdb=" + imdb);
    ...
    a.setAttribute(
      "href",
      `https://embed.smashystream.com/status.php?imdb=${imdb}&season=${season}&episode=${episode}`
    );
    a.setAttribute("target", "_blank");
    a.click();
  }
